...
public class LiveTrackingActivity extends BaseActivity implements OnMapReadyCallback{
    ...
    boolean isTrackingAvailable = false;
...
//            }, 10000);
    ...
    public void initLiveTracking(long userID){
        ...
        mDatabase = FirebaseDatabase.getInstance().getReference()
                .child("tracking").child(String.valueOf(userID)).orderByChild("timestamp").getRef();
            ...
            try {
                ...
                valueEventListener =  new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        //Log.v(Constants.TAG, "onDataChange:" + dataSnapshot.getKey());

                        hideProgressBar();

                        if(dataSnapshot.getValue() != null){
                            for (DataSnapshot child : dataSnapshot.getChildren()) {
                                //Log.v(Constants.TAG,"child: "+child);
                                Tracking tracking = child.getValue(Tracking.class);
                                SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");

                                if(sdf.format(new Date()).equals(sdf.format(tracking.timestamp))){
                            ...
                            if(!isTrackingAvailable){
                                new MaterialDialog.Builder(mContext)
                                        .title("Tracking")
                                        .content("No tracking data for current day")
                                        .positiveText("Ok")
